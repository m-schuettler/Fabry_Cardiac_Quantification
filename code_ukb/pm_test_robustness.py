...
def segment_pm_2D_v2(images, template=[], namelist=[], crop=True, show_bloodpool=False, sort=True, show_images=True):
    ...
    for i, img in zip(range(0, len(imgs)), imgs):
        ...
        bloodpool = ski.morphology.binary_erosion(bloodpool, footprint=np.ones(shape=(1, 1)))
...
#     if not show_images:
#         # fig, axes = plt.subplots(nrows=len(ids), figsize=(7, 1.5*len(ids)), layout='compressed', sharex=True)
#         fig = plt.figure(figsize=(7, 1.5*len(ids)))
#         gs = gridspec.GridSpec(nrows=len(ids), ncols=1)
#         axs = [fig.add_subplot(gs[i]) for i in range(len(ids))]
...
#                 ax = axs[i]
#                 ax.imshow(np.full([10, 50], np.nan))
#                 ax.set_xticks([])
#                 ax.set_yticks([])
#             continue
#         crop = crop[0]
...
#                     area = props[0].area
#                     Q = (4*pi*area)/(perimeter**2)  # isometric quotient
#                 except (RuntimeWarning, IndexError):
#                     Q = 0
#                 if Q < 0.6 or Q > 1 or np.sum(bloodpool) < 100 or np.sum(bloodpool) > 900:
#                     drop.append(j)
#                 # print('iq:', Q)
...
#             ax = axs[i]
#             if i == 0:
#                 ax.set_ylabel('Z Slice', loc='top')
#             # plt.gca().set_title(str(this_id)+' ('+str(i)+'/'+str(len(ids))+')', fontsize='small', loc='left')
#             ax.set_title(str(this_id), fontsize='medium', loc='left')
#             heatmap = ax.imshow(these_pm_areas)
#             # cax = plt.gca().inset_axes((1.05, 0, 0.08, 1.0))
#             plt.colorbar(heatmap, ax=ax, pad=0.01, label='[mm²]',)
#             # plt.colorbar(heatmap, cax=cax, pad=0.01, label='[mm²]')
...
#             if i+1 == len(ids):
#                 ax.set_xlabel('T Slice', loc='right')
#             else:
#                 ax.set_xticks([])
...
#         if not show_images:
#             ax.set_title('SD = '+str(np.round(stdev(neighborhood), 2))+' mm²', fontsize='medium', loc='right')
#             if t_start > t_end:
